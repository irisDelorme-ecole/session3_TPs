...
def contours(image):#ontours avec amplitude du gradient de sobel bidirectionnel, avec seuillage
...
def construire(image):#traite la reponse de l'utilisateur et l'envoie a l'affichage
        ...
        affichage_final(image, convolution_blur(image), gradient_x(image), gradient_y(image), gradient_both(image), contours(image))
